...
<script type="text/javaScript" language="javascript" defer="defer">
		...
		/* 글 수정 화면 function  */
		function fn_egov_select(qandano) {
			document.listForm.selectedQandaNo.value = qandano;
			document.listForm.action = "<c:url value='/updateQandaBorderView.do'/>";
			document.listForm.submit();
		} 

		/* 글 등록 화면 function */
		function fn_egov_addView() {
			document.listForm.action = "<c:url value='/addQandaBorderView.do'/>";
			document.listForm.submit();
		}
...
<form:form commandName="searchVO" id="listForm" name="listForm"
		method="post">		
<input type="hidden" name="selectedQandaNo" />
 ...
 <div class="btn-group">
			...
			<tbody>
				  <c:forEach var="result" items="${resultList}" varStatus="status">
           			...
           			<%-- 	<td align="center" class="listtd"><c:out value="${paginationInfo.totalRecordCount+1 - ((searchVO.pageIndex-1) * searchVO.pageSize + status.count)}"/></td> --%>
           				<td  class="listtd">
           				<a href="javascript:fn_egov_select('<c:out value="${result.qandano}"/>')"><c:out value="${result.qandano}"/></a>
           				</td>
           				<td  class="listtd"><c:out value="${result.question}"/>&nbsp;</td>
           				<td  class="listtd"><c:out value="${result.answer}"/>&nbsp;</td>
	...
	<p style="text-align: center">
      <button class="btn_1" onclick="fn_egov_addView()">질문 등록하기</button>
	</p>
		<br>

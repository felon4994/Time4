...
<script type="text/javaScript" language="javascript" defer="defer">
		...
		/* 글 수정 화면 function  */
		function fn_egov_select(userno) {
			document.listForm.selectedUserNo.value = userno;
			document.listForm.action = "<c:url value='/updateUserBorderView.do'/>";
			document.listForm.submit();
		} 

		/* 글 등록 화면 function */
		function fn_egov_addView() {
			document.listForm.action = "<c:url value='/addUserBorderView.do'/>";
			document.listForm.submit();
		}
...
<form:form commandName="searchVO" id="listForm" name="listForm"
		method="post">		
<input type="hidden" name="selectedUserNo" />
 ...
 <div class="btn-group">
			...
			<tbody>
				  <c:forEach var="result" items="${resultList}" varStatus="status">
           			...
           			<%-- 	<td align="center" class="listtd"><c:out value="${paginationInfo.totalRecordCount+1 - ((searchVO.pageIndex-1) * searchVO.pageSize + status.count)}"/></td> --%>
           				<td  class="listtd">
           				<a href="javascript:fn_egov_select('<c:out value="${result.userno}"/>')"><c:out value="${result.userno}"/></a>
           				</td>
           				<td  class="listtd"><c:out value="${result.title}"/>&nbsp;</td>
	...
	<p style="text-align: center">
      <button class="btn_1" onclick="fn_egov_addView()">게시글 등록하기</button>
	</p>
		<br>

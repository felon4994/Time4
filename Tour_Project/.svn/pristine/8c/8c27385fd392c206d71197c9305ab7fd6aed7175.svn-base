package egovframework.example.sample.service.impl;


import java.util.List;

import egovframework.example.sample.service.EgovSampleService;
import egovframework.example.sample.service.SampleDefaultVO;
import egovframework.example.sample.service.SampleVO;
import egovframework.example.sample.service.Big_CityService;
import egovframework.example.sample.service.Big_CityVO;
import egovframework.example.sample.service.User_BorderService;
import egovframework.example.sample.service.User_BorderVO;

import egovframework.rte.fdl.cmmn.EgovAbstractServiceImpl;
import egovframework.rte.fdl.idgnr.EgovIdGnrService;

import javax.annotation.Resource;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.stereotype.Service;



@Service("userborderService")
public class User_BorderServiceImpl extends EgovAbstractServiceImpl implements User_BorderService{

	private static final Logger LOGGER = LoggerFactory.getLogger(User_BorderServiceImpl.class);

	/** SampleDAO */
	// TODO ibatis 사용
	@Resource(name = "userborderDAO")
	private User_BorderDAO userborderDAO;
	// TODO mybatis 사용
	//  @Resource(name="sampleMapper")
	//	private SampleMapper sampleDAO;

	/** ID Generation */
	@Resource(name = "egovIdGnrService")
	private EgovIdGnrService egovIdGnrService;

	/**
	 * 글 목록을 조회한다.
	 * @param searchVO - 조회할 정보가 담긴 VO
	 * @return 글 목록
	 * @exception Exception
	 */
	@Override
	public List<?> selectUser_BorderList(SampleDefaultVO searchVO) throws Exception {
		return userborderDAO.selectUser_BorderList(searchVO);
	}

	/**
	 * 글을 조회한다.
	 * @param vo - 조회할 정보가 담긴 SampleVO
	 * @return 조회한 글
	 * @exception Exception
	 */
	@Override
	public User_BorderVO selectUser_Border(User_BorderVO vo) throws Exception {
		User_BorderVO resultVO = userborderDAO.selectUser_Border(vo);
		if (resultVO == null)
			throw processException("info.nodata.msg");
		return resultVO;
	}

	/**
	 * 글 총 갯수를 조회한다.
	 * @param searchVO - 조회할 정보가 담긴 VO
	 * @return 글 총 갯수
	 * @exception
	 */
	@Override
	public int selectUser_BorderListTotCnt(SampleDefaultVO searchVO) {
		return userborderDAO.selectUser_BorderListTotCnt(searchVO);
	}
	
	/**
	 * 글을 등록한다.
	 * @param vo - 등록할 정보가 담긴 SampleVO
	 * @return 등록 결과
	 * @exception Exception
	 */
	@Override
	public void insertUser_Border(User_BorderVO vo) throws Exception {
		/*
		LOGGER.debug(vo.toString());

		*//** ID Generation Service *//*
		String deptno = egovIdGnrService.getNextStringId();
		vo.setDeptno(deptno);
		LOGGER.debug(vo.toString());
*/
		userborderDAO.insertUser_Border(vo);
	}
	/**
	 * 글을 수정한다.
	 * @param vo - 수정할 정보가 담긴 SampleVO
	 * @return void형
	 * @exception Exception
	 */
	@Override
	public void updateUser_Border(User_BorderVO vo) throws Exception {
		userborderDAO.updateUser_Border(vo);
	}

	/**
	 * 글을 삭제한다.
	 * @param vo - 삭제할 정보가 담긴 SampleVO
	 * @return void형
	 * @exception Exception
	 */
	@Override
	public void deleteUser_Border(User_BorderVO vo) throws Exception {
		userborderDAO.deleteUser_Border(vo);
	}
	
}
